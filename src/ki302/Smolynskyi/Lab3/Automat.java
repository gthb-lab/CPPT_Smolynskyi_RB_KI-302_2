...
/**
 * Абстрактний клас {@code Automat} моделює базову автоматичну зброю.
 * <p>
 * Він слугує суперкласом для конкретних типів зброї.
 * Визначає основні компоненти:
 * <ul>
...
public abstract class Automat {
    ...
    /**
     * Об'єкт для запису у файл журналу.
     * Доступний для підкласів.
     */
    ...
    public Automat(Barrel barrel, Trigger trigger, Magazine magazine) throws IOException {
        ...
        // Додаємо 'true' для автоматичного скидання буфера (auto-flush)
        logWriter = new PrintWriter(new FileWriter("automat_log.txt", true), true);
        log("Абстрактний клас 'Автомат' створено");
    }

    /**
     * Абстрактний метод для отримання типу зброї.
     * Має бути реалізований у всіх підкласах.
     *
     * @return Рядок, що описує тип зброї.
     */
    ...
    public void fireSingle() {
        ...
        } else if (!magazine.hasBullets()) {
            log("Неможливо здійснити постріл: магазин порожній");
        } else {
            log("Неможливо здійснити постріл: курок не натиснуто");
        }
    }
    ...
    protected void log(String message) {
        ...
        // logWriter.flush(); // Не потрібно, якщо auto-flush увімкнено в конструкторі
    }
}